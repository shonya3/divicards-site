...
export class VerifyPage extends SignalWatcher(LitElement) {
  ...
  #sources_and_cards = computed(() => {
    const sourcesAndCards = cardsBySourceTypes(Array.from(SOURCE_TYPE_VARIANTS), divcord_store.records.get(), poeData)
      .map(({ cards, source }) => ({
  ...
  #by_category = computed<{
    ...
    return {
      ...
      other: cards.filter(({ source }) => ["Act", "Map", "Act Boss", "Map Boss"].every((type) => type !== source.type)),
    };
  });
...
function weightsTableData(records: DivcordRecord[], poeData: PoeData): RowData[] {
  return Object.entries(groupBy(records, ({ card }) => card))
    .map(([card, records]) => ({
      ...
      sources: records.flatMap((record) => record.verifySources.filter(({ type }) => type === "Map" || type === "Act")),
    }))
