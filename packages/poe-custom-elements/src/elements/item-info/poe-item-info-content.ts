    ...
    return html`<div class="content">
      ${[
        ...
        this.requirements.length
          ? html` <poe-item-info-requirements .requirements=${this.requirements}></poe-item-info-requirements>`
          : nothing,
        ...
        this.item.flavourText
          ? html`${this.item.flavourText.map((line, i, arr) => {
              if (i === arr.length - 1 && line.includes("<default>")) {
                return html`<p class="flavour-text_default mt-16">${line.match(/{(.*?)}/)?.[1] ?? ""}</p>`;
              } else {
