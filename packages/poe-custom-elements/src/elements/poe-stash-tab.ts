...
export class PoeStashTabElement extends LitElement {
  ...
  protected render(): TemplateResult {
    ...
    if (!SUPPORTED_STASH_TYPES.includes(this.tab.type)) {
      this.style.setProperty("border", "2px solid red");
      return html`<p style="color: red; font-size: 24px">StashType ( ${this.tab.type} ) is not supported ( yet? ).</p>`;
    }
    return html`
      <ul>
        ${this.#tab.items.map(
          (item) =>
            ...
            >
              <poe-item
                ...
                .item=${item}
                class=${classMap({
                  "item--visible": item.baseType.toLowerCase().includes(this.search_query_for_divination_stash),
                })}
              ></poe-item>
            </li>`,
        )}
...
/**
 * Adjusts the (x, y) coordinates of items in stash tabs that have a custom, non-grid layout, rearranging them to fit
 * into a grid layout.
 *
...
async function item_element_from_x_y({
  ...
  return (
    stash_tab_element.shadowRoot?.querySelector<PoeItemElement>(`poe-item[data-x="${item.x}"][data-y="${item.y}"]`) ??
    null
  );
}
