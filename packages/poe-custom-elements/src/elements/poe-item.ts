...
export class PoeItemElement extends LitElement {
   ...
   *
   * If set to true, sockets are always visible. If set to false, sockets are only visible when the Alt key is pressed
    ...
    return html`
      ...
      ${this.item.stackSize
        ? html`<p
            class=${classMap({
              stackSize: true,
              maxed: frameKind(this.item.frameType) === "divination" && this.item.stackSize === this.item.maxStackSize,
            })}
          >
  ...
  protected firstUpdated(): void {
    SimpleTooltip.lazy(this, (tooltip) => {
      render(
        html`<div style="display:flex;align-items:flex-start;flex-wrap:wrap;gap:1.2rem;z-index:500;padding:0;margin:0">
  ...
  transform(): string {
    return [
      ...
      this.requirements.length
        ? `Requirements: \n${this.requirements.map(({ name, values }) => `${name}: ${values[0][0]}`).join("\n")}`
        : "",
      this.sockets.length
